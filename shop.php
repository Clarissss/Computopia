  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    itemDetailButtons.forEach(button => {
      button.addEventListener('click', function(event) {
        ...
        // Kirim permintaan AJAX untuk mendapatkan detail produk berdasarkan ID produk
        fetch('get_product_detail.php?product_id=' + productId)
          .then(response => response.json())
          .then(product => {
            // Isi konten modal dengan detail produk
            modalProductName.innerText = product.name;
            modalProductDescription.innerText = product.description;
            modalProductPrice.innerText = 'IDR ' + formatCurrency(product.price);
            modalProductImage.src = 'img/products/' + product.photo;
